...
def query_perplexity(prompt, conversation_history=None):
    ...
    messages = [{
        ...
        "content": (
            ...
            "Always provide 3-4 specific, actionable recommendations. Use conversation history for context in follow-ups. "
            "Keep it engaging and avoid dry analysis—add a bit of flair!"
            "Provide the tabular structure when and where possible and keeps the answers clean and visually appealling"
        )
    }]
...
#         context, columns, filters, pivot_table, filtered_df, is_trend_query, is_loser_query = generate_data_context(
#             df, full_query, [], preset_filters
#         )

#         # Prefer a concise, user-friendly summary when we can determine the answer deterministically
#         # Try cost-driver summary first if cost columns present; else top-business summary
#         summary = build_cost_driver_summary(pivot_table, filters) or build_top_business_summary(pivot_table, filters)
#         if summary:
#             response_text = summary
#         else:
#             full_prompt = (
#                 "Answer concisely for a business stakeholder in 2-4 bullet points, "
#                 "avoid preambles, avoid markdown tables, prefer compact numbers (k/M): "
#                 f"{req.message}\n\n{context}"
#             )
#             response_text = query_perplexity(full_prompt, [])
...
# -------- Helper: Universal Response Formatter -------- #
def format_ai_response(raw_text: str) -> str:
    """
    Universal formatter for any AI output.
    It improves readability while keeping the message flexible
    ...
    # Convert plain section titles into readable headers
    text = re.sub(
        r"(?i)\b(cost drivers analysis|key insights|recommendations|summary|conclusion|overview|analysis|findings|trends|insights|observations)\b",
        lambda m: f"\n\n### {m.group(1).title()}\n",
        text,
    )

    # Add line breaks after sentences for spacing
    text = re.sub(r"(?<=[.!?])\s+(?=[A-Z])", "\n\n", text)

    # Turn numeric or dash lists into bullets
    text = re.sub(r"(?m)^\s*(?:[-*]|\d+\.)\s*", "• ", text)

    # Add emoji headers if applicable
...
async def chat(request: ChatRequest):
    try:
        ...
        # ✅ Format the AI output for readability
...
async def insights_chat_alias(req: InsightsRequest):
    try:
        ...
        preset_filters = {}
        ctx = req.context or {}
        year_val = ctx.get('year') or (ctx.get('selectedYears') or [None])[0]
        if year_val:
            try:
                preset_filters['Year'] = int(year_val)
            except Exception:
        ...
        if months:
            preset_filters['Month Name'] = str(months[0])
        business = ctx.get('business') or (ctx.get('selectedBusinesses') or [None])[0]
        if business:
            preset_filters['Business'] = str(business)
